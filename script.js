...
function setupRoomlistSections() {
    if (getByDataHook('lbl-opciones')) return;
    const buttons = body.querySelector('.roomlist-view .buttons');
    const listscroll = getByDataHook('listscroll');
    if (!buttons || !listscroll) return;
    const dialog = buttons.closest('.dialog') || listscroll.closest('.dialog');
    if (!dialog) return;

    const lblOpciones = document.createElement('div');
    lblOpciones.setAttribute('data-hook', 'lbl-opciones');
    lblOpciones.className = 'roomlist-section-label';
    lblOpciones.innerHTML = 'OPCIONES';

    const lblSalas = document.createElement('div');
    lblSalas.setAttribute('data-hook', 'lbl-salas');
    lblSalas.className = 'roomlist-section-label';
    lblSalas.innerHTML = 'SALAS';

    const creditos = document.createElement('div');
    creditos.setAttribute('data-hook', 'sec-creditos');
    creditos.innerHTML = '<div class="roomlist-section-label">CREDITOS</div><div class="roomlist-creditos-text">⚡ RayoClient - Haxball Mobile</div>';

    dialog.appendChild(lblOpciones);
    dialog.appendChild(buttons);
    dialog.appendChild(lblSalas);
    dialog.appendChild(listscroll);
    dialog.appendChild(creditos);
}
...
function createURLButton() {
    ...
    button.innerHTML = '<i class="icon-link"></i><div>URL</div>';
